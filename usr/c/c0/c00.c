...
int decl1(int askw, int tkw, int offset, int elsize);
void decsyn(int o);
void redec();
...
loop:
    ...
    case ASSIGN:
        ...
        if (c >= PLUS && c <= EXOR)
        {
            ...
            if (ctab[peekc] != SPACE && (c == MINUS || c == AND || c == TIMES))
            {
...
advanc:
    ...
    case NAME:
        ...
        *cp++ = block(2, NAME, cs->htype, cs->hdimp, cs->hclass, 0, 0);
    ...
    case CON:
    case SFCON:
        *cp++ = block(1, o, (o == CON ? INT : DOUBLE), 0, cval, 0, 0);
        goto tand;
...
opon1:
    ...
    case MCALL:
        *cp++ = block(0, 0, 0, 0, 0, 0, 0); /* 0 arg call */
        os = CALL;
        goto fbuild;
...
int decl1(int askw, int tkw, int offset, int elsize)
{
    ...
    if (!(dsym->hclass == 0 || (skw == ARG && dsym->hclass == ARG1) || (skw == EXTERN && dsym->hclass == EXTERN && dsym->htype == type)))
    {
    ...
    {
        if ((type & 07) > CHAR && (type & 030) == 0 || (type & 030) > PTR || regvar < 3)
